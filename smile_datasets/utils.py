...
def bucketing_and_padding(dataset: tf.data.Dataset, bucket_fn, padded_shapes, padding_values, **kwargs) -> tf.data.Dataset:
    batch_size = kwargs.get("batch_size", 32)
    ...
    assert len(bucket_batch_sizes) == len(bucket_boundaries) + 1, "len(bucket_batch_sizes) != len(bucket_boundaries) + 1"
